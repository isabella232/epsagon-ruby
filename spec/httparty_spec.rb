...
RSpec.describe 'HTTParty Instrumentation' do
  ...
  describe 'PUT' do
    context 'with metadata_only = true and no body' do
      let(:metadata_only) { true }
      before do
        HTTParty.put('http://localhost/put', headers: request_headers)
      end

      it_behaves_like 'HTTP Metadata Only', 'http', 'PUT', 'http://localhost/put'
      it_behaves_like 'HTTP Metadata Only Non-Present Fields'
    end
    ...
    context 'with metadata_only = true and body' do
      ...
      before do
        HTTParty.put('http://localhost/put', headers: request_headers, body: body.to_json)
      end
